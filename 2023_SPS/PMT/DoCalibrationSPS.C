...
void DoCalibrationSPS(){
	...
	*/
	...
	int counter2=0;
	// loop on ntuple entries to get adc peak position in T0 for T0-T8 equalization 
	...
	myAdc_s.push_back(adc_SSIPM); // Peak Position of (H1-H8,H0)
	myAdc_c.push_back(adc_CSIPM); // Peak Position of (H1-H8,H0)

	// Calculate equalization constants for H1-H8 and H0(=1) and dump to equalisation.txt
	...
	/**
	 * Start calorimeter calibration to rescale the response
	 * of each module, so that the total energy corresponded
	 * to the beam energy separately for the scin and Cher.
	*/
	...
	// Calculate equalization constants * scaling factor for Modules and dump to pmt_json.txt
