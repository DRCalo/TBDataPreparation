...
public:    
  ...
  void SetDoEventBuilding(bool l_doEventBuilding) {m_doEventBuilding = l_doEventBuilding;}
  void SetEventBuildingMode(unsigned int l_eventBuildingMode) {m_finfo.SetEventBuildingMode(l_eventBuildingMode);} // if 1, then build based on TrigID. If 2, then build based on the FERS time stamp 
...
private: 
  ...
  bool m_doEventBuilding; // if true attempt building events from event fragments
