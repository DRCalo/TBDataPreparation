...
// Reviewed again for 2025 test beam 
//**************************************************
//
...
void PhysicsConverter(const string run, const string inputPath, const string calFile, bool doCalibration, bool doLocPed, const string adcMapFile){
  ...
  string infile = inputPath+"merged_sps2025_run"+run+".root"; // Actually we can also merge the merged_sps2023_run into inputPath
  ...
  TFile * Mergfile = new TFile(infile, "READ");
  TTree * PMTtree = (TTree*) Mergfile->Get("CERNSPS2025");
